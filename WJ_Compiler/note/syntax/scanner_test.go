...
// 只是scanner最基本的一个单元测试,如果这个测不过,其他的页不用测了.
func TestSmoke(t *testing.T) {
	...
	// s: "if (+foo\t+=..123/***/0.9_0e-0i'a'`raw`\"string\"..f;//$",
	// i: 0,
	//prevRune: -1,}

    
    // *command-line-arguments.source {
	// in: io.Reader(*strings.Reader) *{
		// s: "if (+foo\t+=..123/***/0.9_0e-0i'a'`raw`\"string\"..f;//$",
		// i: 0,
		// prevRune: -1,},
	// errh: nil,
	...
	for _, want := range tokens {
        // 开始分词,每次从got.next跳出都应该是匹配到对应的预期值 
		got.next()
        //不匹配就测试不通过 
		if got.tok != want {
...
func TestScanner(t *testing.T) {
	...
	for {
		...
		case _Operator:
		    fmt.Printf("%s:%d:%d: %s => %s (prec = %d)\n", filename, s.line, s.col, s.tok, s.op.String(), s.prec)
		default:
